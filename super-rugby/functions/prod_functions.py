...
def get_data():
    ...
    historic = (pd.read_excel('http://www.aussportsbetting.com/historical_data/super_rugby.xlsx', header=1)[relevant_cols]
...
def create_json_predictions(predictions_df, game_week):
    json_preds = {
        "tournaments": [
            {
                "name": "Super Rugby",
                "title": "Super Rugby Predictions".format(game_week),
                "season": "2019",
                                ...
                                "picks": [
                                    {
                                        ...
                                        "model": str(round(g2.groupby('elo_odds_1')['elo_odds_1'].sum().values[0], 2)),
                                        "line": "+0"
                                    },
                                    {
                                        "bfExchangeSelectionId": str(g2.groupby('selection_id_2')['selection_id_2'].sum().values[0]),
                                        "model": str(round(g2.groupby('elo_odds_1')['elo_odds_2'].sum().values[0], 2)),
                                        "line": "+0"
                                    }
                                ]
